...
if train:
    ...
    model.save('RSGTask/RL/Models/RSG_RL_Models/'+ModelDir+'.zip')
    print("Done")
...
if runTrained:
    ...
    envRunTrained = monitor.Monitor(envRunTrained, folder='RSGTask/RL/Plots/RunTrained/', sv_per=numSteps, verbose=False, sv_fig=True, num_stps_sv_fig=3500)
    envRunTrained = DummyVecEnv([lambda: envRunTrained])

    # Define Model
    modelTrained = A2C.load(load_path = '/Users/ritikmehta/School/Bsc_NB3/BEP/neurogym/RSGTask/RL/Models/RSG_RL_Models/IN0.5%_TT1.0%/30Cycles_1e-05LR.zip', 
                            env = envRunTrained)
