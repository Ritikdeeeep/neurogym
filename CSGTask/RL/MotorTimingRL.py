...
if run:
    ...
    envRun = monitor.Monitor(envRun, folder='CSGTask/Plots/CSG_RL/Run/'+ModelDir, 
...
if train:
    ...
    envTrain.reset()
    envTrain = monitor.Monitor(envTrain, folder='CSGTask/Plots/CSG_RL/Train/', 
                                sv_per=numSteps, verbose=False, sv_fig=True, num_stps_sv_fig=numSteps)
    ...
    model = A2C(LstmPolicy, envTrain, verbose=1, 
                ...
                tensorboard_log="CSGTask/Models/CSG_RL_Tensorboard/",
    ...
    model.save('CSGTask/Models/CSG_RL_Models/'+ModelDir+'.zip')
    print("Done")
...
if runTrained:
    ...
    envRunTrained = monitor.Monitor(envRunTrained, folder='CSGTask/Plots/CSG_RL/RunTrained/', sv_per=numSteps, verbose=False, sv_fig=True, num_stps_sv_fig=3500)
    envRunTrained = DummyVecEnv([lambda: envRunTrained])

    # Define Model
    modelTrained = A2C.load(load_path = '/Users/ritikmehta/School/Bsc_NB3/BEP/neurogym/CSGTask/Models/CSG_RL_Models/IN0.5%_TT1.0%/30Cycles_1e-05LR.zip', 
                            env = envRunTrained)
