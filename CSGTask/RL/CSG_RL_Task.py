...
class CSG_RL_RT(ngym.TrialEnv):
    ...
    def _new_trial(self, Scenario=None, WaitTime=None, **kwargs):
        ...
        self.trial_duration = self.burn + self.wait_time + self.set + trial['production'] + self.threshold_delay + self.BP_delay
        ...
        self.add_period('production', duration=self.trial_duration-(self.BP_delay+self.set+self.wait_time+self.burn), after='set')
        ...
        if self.target_ramp == False:      
            ...
            gt[self.burn+self.wait_time+self.set+int(trial['production']):self.trial_duration-self.BP_delay] = 1
