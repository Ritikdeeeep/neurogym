...
##################################################################### Done
def random_vrnn_params_DS(key, u, n, o, g=1.0):
...
def random_vrnn_params(u, n, o, g=1.0):
  """Generate random RNN parameters"""

  hscale = 0.1
  ifactor = 1.0 / onp.sqrt(u)
  hfactor = g / onp.sqrt(n)
  pfactor = 1.0 / onp.sqrt(n)
  return {'h0' : onp.random.normal(0, hscale, size=(n,)),
          'wI' : onp.random.normal(0, ifactor, size=(n,u)),
          'wR' : onp.random.normal(0, hfactor, size=(n,n)),
          'wO' : onp.random.normal(0, pfactor, size=(o,n)),
          'bR' : onp.zeros([n]),
          'bO' : onp.zeros([o])}

##################################################################### Done
def affine_DS(params, x):
  """Implement y = w x + b"""
  return np.dot(params['wO'], x) + params['bO']

def affine(params, x):
  """Implement y = w x + b"""
  return onp.dot(params['wO'], x) + params['bO']
...
def vrnn(params, h, x):
  """Run the Vanilla RNN one step"""
  a = onp.dot(params['wI'], x) + params['bR'] + onp.dot(params['wR'], h)
  return onp.tanh(a)

##################################################################### Done
def vrnn_scan_DS(params, h, x):
...
##################################################################### Not Done
def vrnn_run_with_h0_DS(params, x_t, h0):
...
def plot_params(params):
  ...
  plt.subplot(234)
  plt.stem(params['wI'][0], label='Input 1', linefmt ='blue')
  plt.stem(params['wI'][1], label='Input 2', linefmt ='red')
