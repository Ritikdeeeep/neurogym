...
class MotorTiming(ngym.TrialEnv):
    ...
    def _new_trial(self, scenario=None, **kwargs):
        # Define Times
        self.waitTime = int(self.rng.uniform(100, 200))
        self.burn = 50
        self.spike = 10
    ...
    def _step(self, action):
        ...
        if self.in_period('production'):
            if  action >= 0.90: # Action is over Threshold
                ...
                eps_threshold = int(trial[0]['production']*self.TargetThreshold) # Allowed margin to produced interval
            ...
            if self.ThresholdReward == True:
                ...
                if self.TimeAfterThreshold >= self.ThresholdDelay: # Give reward ThresholDelay steps after Success
                    new_trial = True
                    self.ThresholdReward = False

            if self.t > self.trialDuration:
                new_trial = True
        ...
        return ob, reward, new_trial, {
            ...
            'SetStart': self.spike+trial[0]['production']+self.spike+self.waitTime+self.burn,
            }
